...
private:
	...
	void AddNextRoomNode(TSharedPtr<FIsaacRoomFactory> roomFactory, TQueue<FLevelGrowthNode>& queue, TSet<FIntVector>& visited, int maxRooms, FIsaacRoomPtr parentRoom, int direction, float probability);
	...
	void CreateMarkerAt(float gridX, float gridY, float angle, const FString& markerName, const FVector& roomBasePosition, const FVector& tileSize);
	void EmitBoundaryMarkers(const FString& markerName, EIsaacRoomTileType adjacentTile1, EIsaacRoomTileType adjacentTile2,
