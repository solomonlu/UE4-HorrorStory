...
public:
	...
	static TArray<T*> CreateInstances(TArray<TSubclassOf<T>> TClasses) {
		...
		for (TSubclassOf<T> TClass : TClasses) {
			if (!TClass) continue;
			T* t = NewObject<T>((UObject*)GetTransientPackage(), TClass);
			if (t) {
				Result.Add(t);
			}
		}
	...
	static bool LineIntersectsLine(const FIntVector& l1p1, const FIntVector& l1p2, const FIntVector& l2p1, const FIntVector& l2p2);
