...
#include "IsaacDungeonSelectorLogic.h"
#include "IsaacDungeonTransformLogic.h"
#include "IsaacDungeonToolData.h"
#include "IsaacDungeonModel.h"
#include "IsaacDungeonConfig.h"

#include <stack>
#include <sstream>
#include <queue>
#include "Utils/IsaacBuilderUtils.h"
...
void UIsaacDungeonBuilder::GenerateLevelLayout()
{
	...
	FIsaacRoomFactoryPtr roomFactory = MakeShareable(new FIsaacRoomFactory(isaacConfig->RoomWidth, isaacConfig->RoomLength));
	rooms.Reset();
	doors.Reset();
	...
	while (!queue.IsEmpty())
	{
		...
		if (isSpawnRoom)
		{
			...
			for (int d = 0; d < 4; d++)
			{
				AddNextRoomNode(roomFactory, queue, visited, numRooms, top.room, d, isaacConfig->SpawnRoomBranchProbability);
			}
			isSpawnRoom = false;
		}
		else
		{
			// Grow forward
			AddNextRoomNode(roomFactory, queue, visited, numRooms, top.room, top.moveDirection, isaacConfig->GrowForwardProbability);

			// Grow sideways
			AddNextRoomNode(roomFactory, queue, visited, numRooms, top.room, (top.moveDirection + 1) % 4, isaacConfig->GrowSidewaysProbability);
			AddNextRoomNode(roomFactory, queue, visited, numRooms, top.room, (top.moveDirection + 3) % 4, isaacConfig->GrowSidewaysProbability);
		}
	...
	for(auto room : rooms)
	{
		room->Layout = isaacConfig->RoomLayoutBuilder->GenerateLayout(room, random, isaacConfig->RoomWidth, isaacConfig->RoomLength);
	}
...
void UIsaacDungeonBuilder::AddNextRoomNode(TSharedPtr<FIsaacRoomFactory> roomFactory, TQueue<FLevelGrowthNode>& queue, TSet<FIntVector>& visited,
...
public:
	virtual FIsaacRoomLayout GenerateLayout(FIsaacRoomPtr room, FRandomStream& random, int roomWidth, int roomHeight) = 0;
};
...
public:
	...
	FIsaacRoomLayout GenerateLayout(FIsaacRoomPtr room, FRandomStream& random, int roomWidth, int roomHeight)
	{
	...
	for(const FIsaacRoom& room : isaacModel->Rooms)
	{
		...
		for (int x = -1; x < roomWidth + 1; x++)
		{
			for (int y = -1; y < roomHeight + 1; y++)
			{
				...
				{
					CreateMarkerAt(x + 0.5f, y + 0.5f, 0, ST_GROUND, roomBasePosition, tileSize);
				}

			}
		}

		EmitBoundaryMarkers(ST_WALL, EIsaacRoomTileType::Empty, EIsaacRoomTileType::Floor, room, roomWidth, roomHeight, roomBasePosition, tileSize);
		EmitBoundaryMarkers(ST_DOOR, EIsaacRoomTileType::Door, EIsaacRoomTileType::Floor, room, roomWidth, roomHeight, roomBasePosition, tileSize);

	}
}

void UIsaacDungeonBuilder::EmitBoundaryMarkers(const FString& markerName, EIsaacRoomTileType adjacentTile1, EIsaacRoomTileType adjacentTile2, 
		const FIsaacRoom& room, int32 roomWidth, int32 roomHeight, const FVector& roomBasePosition, const FVector& tileSize)
{
...
void UIsaacDungeonBuilder::CreateMarkerAt(float gridX, float gridY, float angle, const FString& markerName, const FVector& roomBasePosition, const FVector& tileSize)
{
...
bool UIsaacDungeonBuilder::ProcessSpatialConstraint(UDungeonSpatialConstraint* SpatialConstraint, const FTransform& Transform, FQuat& OutRotationOffset)
{
	return false;
}
...
void UIsaacDungeonBuilder::GetDefaultMarkerNames(TArray<FString>& OutMarkerNames)
{
	...
	OutMarkerNames.Add(ST_GROUND);
	OutMarkerNames.Add(ST_WALL);
	OutMarkerNames.Add(ST_DOOR);
}
...
bool UIsaacDungeonBuilder::PerformSelectionLogic(const TArray<UDungeonSelectorLogic*>& SelectionLogics, const FPropSocket& socket)
{
	return false;
}

FTransform UIsaacDungeonBuilder::PerformTransformLogic(const TArray<UDungeonTransformLogic*>& TransformLogics, const FPropSocket& socket)
{
