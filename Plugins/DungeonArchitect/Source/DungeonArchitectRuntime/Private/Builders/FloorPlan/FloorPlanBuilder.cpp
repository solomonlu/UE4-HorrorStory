...
#include "FloorPlanSelectorLogic.h"
#include "FloorPlanTransformLogic.h"
#include "FloorPlanToolData.h"
#include "FloorPlanModel.h"
#include "FloorPlanConfig.h"
#include "Volumes/FloorPlanRoomVolume.h"
#include "Volumes/FloorPlanDoorVolume.h"
#include "Volumes/FloorVolumeQuery.h"
...
void UFloorPlanBuilder::SplitChunk(FloorChunkPtr Chunk, FloorChunkPtr OutLeft, FloorChunkPtr OutRight, FloorChunkPtr OutHallway) {
...
{
	FVector WorldLocation = FVector(GridPoint.X + 0.5f, GridPoint.Y + 0.5f, GridPoint.Z + 0.25f) * floorPlanConfig->GridSize;
		...
		for (int x = -1; x < floorPlanConfig->BuildingSize.X; x++) {
			...
			for (int y = -1; y < floorPlanConfig->BuildingSize.Y; y++) {
				...
				*/
				...
				if (Chunk00.IsValid() && Chunk10.IsValid() && Chunk00->ChunkType == FloorChunkType::Hall && Chunk10->ChunkType == FloorChunkType::Hall) {
					// Do not block the halls with a wall
					bEmitLeftWall = false;
				}
				if (Chunk00.IsValid() && Chunk01.IsValid() && Chunk00->ChunkType == FloorChunkType::Hall && Chunk01->ChunkType == FloorChunkType::Hall) {
					// Do not block the halls with a wall
					bEmitBottomWall = false;
				}

				if (Chunk00.IsValid() && Chunk10.IsValid() && (!Chunk00->bEmitGroundMarker || !Chunk10->bEmitGroundMarker)) {
					// We don't have ground in one of the adjacent chunks. Can't have doors
					bLeftDoor = false;
				}
				if (Chunk00.IsValid() && Chunk01.IsValid() && (!Chunk00->bEmitGroundMarker || !Chunk01->bEmitGroundMarker)) {
					// We don't have ground in one of the adjacent chunks. Can't have doors
					bBottomDoor = false;
				}
					...
					if (bLeftDoor) {
						...
						if (bBuildingWallLeft) {
							...
							if (Chunk00.IsValid() && Chunk10.IsValid()) {
								FloorChunkPtr PriorityChunk = (Chunk00->Priority > Chunk10->Priority) ? Chunk00 : Chunk10;
					...
					if (bBottomDoor) {
						...
						if (bBuildingWallBottom) {
							...
							if (Chunk00.IsValid() && Chunk01.IsValid()) {
								FloorChunkPtr PriorityChunk = (Chunk00->Priority > Chunk01->Priority) ? Chunk00 : Chunk01;
								if (PriorityChunk->WallMarker.Len() > 0) {
									MarkerName = PriorityChunk->WallMarker;
								}
							}
...
bool UFloorPlanBuilder::ProcessSpatialConstraint(UDungeonSpatialConstraint* SpatialConstraint, const FTransform& Transform, FQuat& OutRotationOffset)
{
	return false;
}
...
bool UFloorPlanBuilder::PerformSelectionLogic(const TArray<UDungeonSelectorLogic*>& SelectionLogics, const FPropSocket& socket)
{
	for (UDungeonSelectorLogic* SelectionLogic : SelectionLogics) {
		...
		if (!FloorSelectionLogic) {
			UE_LOG(FloorPlanBuilderLog, Warning, TEXT("Invalid selection logic specified.  FloorPlanSelectorLogic expected"));
			return false;
		}
...
FTransform UFloorPlanBuilder::PerformTransformLogic(const TArray<UDungeonTransformLogic*>& TransformLogics, const FPropSocket& socket)
{
	...
	for (UDungeonTransformLogic* TransformLogic : TransformLogics) {
		...
		if (!FloorTransformLogic) {
			UE_LOG(FloorPlanBuilderLog, Warning, TEXT("Invalid transform logic specified.  FloorPlanTransformLogic expected"));
			continue;
		}
...
void FloodFill(const FIntVector& StartLocation, int32 IslandId, TSet<FIntVector>& Visited, FloorIslandNodeList& IslandNodes, FloorChunkDBPtr ChunkDB) {
...
void ConnectIslandRecursive(int32 IslandId, const TMap<int32, TArray<FloorIslandAdjacency>>& AdjacencyByIslands, 
		TSet<int32>& IslandVisited, FRandomStream& Random, FloorDoorManager& DoorManager, TMap<int32, FloorChunkPtr> IslandToChunkMap) {
