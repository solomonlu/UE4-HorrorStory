...
bool UGridDungeonSelectorLogic::IsPillarOnCorner(UGridDungeonModel* Model, int32 GridX, int32 GridY, FTransform& OutCornerOffset)
{
	...
	for (int32 i = 0; i < 4; i++) {
		...
		if (Cell) {
			...
			Hash = Cell->Bounds.Location.Z * 100 + (int32)CellType;
...
{
	if (Model->CellStairs.Contains(Cell.Id)) {
		const TArray<FStairInfo>& Stairs = Model->CellStairs[Cell.Id];
...
bool UGridDungeonSelectorLogic::SelectNode_Implementation(UGridDungeonModel* Model, UGridDungeonConfig* Config, const FCell& Cell, const FRandomStream& RandomStream, int32 GridX, int32 GridY)
{
	return false;
}
