...
#include "DelauneyTriangleGenerator.h"
...
{
	const XYZ *p1 = (const XYZ*)v1;
	const XYZ *p2 = (const XYZ*)v2;
	if (p1->x < p2->x)
		return(-1);
	else if (p1->x > p2->x)
		return(1);
...
void DelauneyTriangleGenerator::PerformTriangulation()
{
	int nv = Points.Num();
	int sizeXYZ = nv + 3;
	XYZ* pxyz = (XYZ*)malloc((nv + 3) * sizeof(XYZ));
	ITRIANGLE* vt = (ITRIANGLE*)malloc((nv * 3) * sizeof(ITRIANGLE));
	int ntri;
