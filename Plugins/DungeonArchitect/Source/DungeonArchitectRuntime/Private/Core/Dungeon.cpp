...
ADungeon::ADungeon(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer), bDrawDebugData(false)
{
	...
	if (!IsRunningCommandlet()) {
		...
		struct FConstructorStatics
		{
			...
			FText NAME_Dungeon;
			FConstructorStatics()
				: DungeonSpriteObject(TEXT("/DungeonArchitect/Textures/S_DungeonActor"))
...
T* CreateNewObject(const FObjectInitializer& ObjectInitializer, UObject* Outer, UClass* ImplClass) {
	...
	UObject* Instance = ObjectInitializer.CreateDefaultSubobject(Outer, FName(*ImplClass->GetName()), T::StaticClass(), ImplClass, true, false, false);
...
void FDungeonBuilderTask::DoWork()
{
	...
	if (DungeonBuilder->SupportsTheming()) {
		...
		DungeonUtils::FDungeonEventListenerNotifier::NotifyMarkersEmitted(Dungeon);

		// Spawn dungeon visual objects (meshes, lights, blueprints etc)
		DungeonBuilder->ApplyDungeonTheme(Dungeon->Themes, SceneProvider, World);
	}
	else {
