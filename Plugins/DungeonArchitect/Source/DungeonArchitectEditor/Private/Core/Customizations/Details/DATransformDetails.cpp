...
FDATransformDetails::FDATransformDetails( const TArray< TWeakObjectPtr<UObject> >& InSelectedObjects, const FSelectedActorInfo& InSelectedActorInfo, IDetailLayoutBuilder& DetailBuilder )
...
FUIAction FDATransformDetails::CreateCopyAction( ETransformField::Type TransformField ) const
{
	return
		FUIAction
		(
...
FUIAction FDATransformDetails::CreatePasteAction( ETransformField::Type TransformField ) const
{
	return 
		...
		if( ObjectPtr.IsValid() )
		{
			...
			if( Node )
			{
				...
				if( OldValue != NewValue )
				{
					...
					UProperty* OffsetProperty = FindField<UProperty>( UEdGraphNode_DungeonActorBase::StaticClass(), "Offset" );
		...
		for( int32 ObjectIndex = 0; ObjectIndex < SelectedObjects.Num(); ++ObjectIndex )
		{
			...
			if( ObjectPtr.IsValid() )
			{
				...
				if(Node)
				{
					...
					FRotator CurrentComponentRotation = FRotator(Node->Offset.GetRotation()); // Intentionally make a copy, we don't want the FRotator& below to directly edit the component's values!
					...
					if( ValueToChange != NewValue )
					{
						...
						FPropertyChangedEvent PropertyChangedEvent(OffsetProperty, !bCommitted && bEditingRotationInUI ? EPropertyChangeType::Interactive : EPropertyChangeType::ValueSet );
