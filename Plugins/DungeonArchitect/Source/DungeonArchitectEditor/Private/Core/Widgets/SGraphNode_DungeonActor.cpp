...
void SGraphNode_DungeonActor::BuildThumbnailWidget()
{
	...
	GetValue(Value);
	AssetThumbnail = MakeShareable(new FAssetThumbnail(Value.AssetData, ThumbnailSize.X, ThumbnailSize.Y, FDungeonEditorThumbnailPool::Get()));
}
...
FSlateColor SGraphNode_DungeonActor::GetIndexColor(bool bHovered) const
{
	...
	return bHovered ? FEditorStyle::Get().GetSlateColor(NodeHoveredColor) : FEditorStyle::Get().GetSlateColor(NodeDefaultColor);
}
...
TArray<FOverlayWidgetInfo> SGraphNode_DungeonActor::GetOverlayWidgets(bool bSelected, const FVector2D& WidgetSize) const
{
