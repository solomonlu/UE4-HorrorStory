...
FDungeonEditorViewportClient::FDungeonEditorViewportClient(TWeakPtr<IDungeonArchitectEditor> pInDungeonEditor, TWeakPtr<SDungeonEditorViewport> pInDungeonEditorViewport, FPreviewScene& pInPreviewScene, UDungeonThemeAsset* pInProp)
...
void FDungeonEditorViewportClient::ProcessClick(class FSceneView& View, class HHitProxy* HitProxy, FKey Key, EInputEvent Event, uint32 HitX, uint32 HitY)
{
	...
	if (HitProxy)
	{
		if (HitProxy->IsA(HActor::StaticGetType()))
		{
			...
			if (ActorProxy && ActorProxy->Actor && ActorProxy->PrimComponent != NULL)
			{
