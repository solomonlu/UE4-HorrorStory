...
void UEdGraphSchema_Extensions::CreateCustomActions(TArray<TSharedPtr<FEdGraphSchemaAction>>& OutActions, const UEdGraph* Graph, UEdGraph* OwnerOfTemporaries, bool bShowNewMesh, bool bShowNewMarker, bool bShowMarkerEmitters)
{
	for (TSharedPtr<DungeonSchemaExtensionProvider> Extension : Extensions) {
		Extension->CreateCustomActions(OutActions, Graph, OwnerOfTemporaries, bShowNewMesh, bShowNewMarker, bShowMarkerEmitters);
	}
}
