...
{	
	...
	UStaticMeshComponent* AdvancedOptionsCopy = NewObject<UStaticMeshComponent>(Outer, NAME_None, RF_NoFlags, AdvancedOptions);
...
void UEdGraphNode_DungeonMesh::PostEditChangeProperty(struct FPropertyChangedEvent& e)
{
	...
	FString PropertyName = e.Property->GetName();
	if (PropertyName == "Mesh") {
		AdvancedOptions->StaticMesh = Mesh;
	}
}
