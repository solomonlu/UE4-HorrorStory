...
void FGridDungeonEdModeRenderer::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI, TSharedPtr<IDungeonEdTool> ActiveTool)
{
	TArray<FIntVector> StrokeData;

	if (ActiveTool.IsValid() && ActiveTool->GetToolFamily() == FGridDungeonEdTool::ToolFamily) {
		auto GridTool = StaticCastSharedPtr<FGridDungeonEdTool>(ActiveTool);  //ActiveTool.IsValid() ? ActiveTool->GetStrokeData() : TArray<FIntVector>();
	...
	FDynamicColoredMaterialRenderProxy* SelectedColorInstance = new FDynamicColoredMaterialRenderProxy(OverlayMaterial->GetRenderProxy(false), SelectedColor);
	...
	if (Dungeon) {
		...
		DrawPaintedCells(StrokeData, GridCellSize, MeshBuilder);
	}
...
void FGridDungeonEdModeRenderer::DrawBorderCursor(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI, FIntVector GridCursorPosition, FDynamicMeshBuilder& MeshBuilder, FIntVector CursorSize, int32 Thickness)
{
	ADungeon* Dungeon = ModeHandler->GetActiveDungeon();
	if (!Dungeon) return;
	UGridDungeonConfig* GridConfig = Cast<UGridDungeonConfig>(Dungeon->GetConfig());
...
void FGridDungeonEdModeRenderer::DrawDungeonCells(UGridDungeonModel* GridModel, const FVector& GridCellSize, FDynamicMeshBuilder& MeshBuilder)
{
...
void FGridDungeonEdModeRenderer::DrawPaintedCells(const TArray<FIntVector>& PaintedCells, const FVector& GridCellSize, FDynamicMeshBuilder& MeshBuilder)
{
	const FColor CellColor = FColor::Green;
	for (const FIntVector& Cell : PaintedCells) {
		FRectangle Bounds;
		Bounds.Location = Cell;
		Bounds.Size = FIntVector(1, 1, 1);
		AddQuad(MeshBuilder, Bounds, GridCellSize, CellColor);
	}
}

void GetQuadVertices(const FRectangle& GridBounds, const FVector& GridCellSize, FVector& V0, FVector& V1, FVector& V2, FVector& V3) {
...
void FGridDungeonEdModeRenderer::AddQuad(FDynamicMeshBuilder& MeshBuilder, const FRectangle& GridBounds, const FVector& GridCellSize, const FColor& Color)
{
	FVector V0, V1, V2, V3;
...
void FGridDungeonEdModeRenderer::AddQuadBorder(FPrimitiveDrawInterface* PDI, const FRectangle& GridBounds, const FVector& GridCellSize, const FColor& Color, float Thickness)
{
	FVector V[4];
...
void FGridDungeonEdModeRenderer::AddQuad(FDynamicMeshBuilder& MeshBuilder, const FVector& V0, const FVector& V1, const FVector& V2, const FVector& V3, const FColor& Color)
{
